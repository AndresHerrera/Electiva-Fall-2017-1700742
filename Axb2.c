...
int distributereceive(int f, int c, int *m, int *x, int size) {
	...
	*/
	...
	for (int xr=0;xr<f;xr++)
	{
		for(int yc=0;yc<c;yc++)
		{
		   row[yc]=m[xr*MAXCOL + yc];
		}
		MPI_Send(&row, c, MPI_INT, target, TAGTAREA, MPI_COMM_WORLD);
		MPI_Recv(&sum,1,MPI_INT,target,TAGRESULT,MPI_COMM_WORLD, &status);
		x[xr]=sum;
		if (target==size-1)
		{
		    target=1;
 	...
 	{
	   MPI_Send(&done, 1, MPI_INT, i, TAGPARAR, MPI_COMM_WORLD);
	}
	return 0;
}
	...
	*/
	...
	do{  
	      	MPI_Recv(iRow,MAXCOL,MPI_INT,0,MPI_ANY_TAG,MPI_COMM_WORLD, &status);           if(status.MPI_TAG==TAGTAREA)
		{
			...
			{
				sum=sum+iVec[k]*iRow[k];
			}
			MPI_Send(&sum,1,MPI_INT,0,TAGRESULT,MPI_COMM_WORLD); 
		}
	}while(status.MPI_TAG!=TAGPARAR);
	return 0;
}
